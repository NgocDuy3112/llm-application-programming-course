"""
Module 6-7 - Memory Management (Solution)
...
class BaseMemory(ABC):
    ...
    def add_tool_message(self, tool_call, content: str):
        ...
        Được gọi sau khi thực thi tool để lưu kết quả vào history,
        giúp LLM hiểu được tool output trong lần gọi tiếp theo.

        Args:
            tool_call: OpenAI ToolCall object với các attributes:
                - .id: Unique tool call identifier
                - .function.name: Tên tool được gọi
            content (str): Kết quả thực thi tool (string hoặc convertible to string)

        Note:
            Tool messages có format đặc biệt với role="tool" và tool_call_id
            để link với tool call ban đầu.
        """
