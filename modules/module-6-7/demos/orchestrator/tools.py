...
def tavily_search(query: str) -> str:
    ...
    try:
        response = tavily_client.search(
            ...
            time_range="year",    # Search results from past year
        )
        if response is None:
            global_logger.warning("Tavily search returned no response")
            return "No response from Tavily"
        # Ensure answer is a string for concatenation
...
# Engine uses this to look up and execute tools by name
AVAILABLE_FUNCTIONS = {
...
# Tool specifications for LLM function calling (OpenAI format)
# These are sent to the LLM API so it knows what tools are available
DEFAULT_TOOLS = [
    {
        "type": "function",
        "function": {
